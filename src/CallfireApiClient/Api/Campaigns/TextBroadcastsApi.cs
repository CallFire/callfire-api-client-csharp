...
namespace CallfireApiClient.Api.Campaigns
{
    public class TextBroadcastsApi
    {
        ...
        {
            String path = TB_BATCHES_ITEM_PATH.ReplaceFirst(ClientConstants.PLACEHOLDER, request.Id.ToString());
            return Client.Get<Batch>(path, request);
        }

        /// <summary>
        /// Update text broadcast batch
        /// </summary>
        /// <param name="batch">batach to update</param>
        /// <exception cref="BadRequestException">          in case HTTP response code is 400 - Bad request, the request was formatted improperly.</exception>
        /// <exception cref="UnauthorizedException">        in case HTTP response code is 401 - Unauthorized, API Key missing or invalid.</exception>
        /// <exception cref="AccessForbiddenException">     in case HTTP response code is 403 - Forbidden, insufficient permissions.</exception>
        /// <exception cref="ResourceNotFoundException">    in case HTTP response code is 404 - NOT FOUND, the resource requested does not exist.</exception>
        ...
        {
            Validate.NotNull(batch.Id, "batch.id cannot be null");
            Client.Put<object>(TB_BATCHES_ITEM_PATH.ReplaceFirst(ClientConstants.PLACEHOLDER, batch.Id.ToString()), batch);
        }
